# EXPERIMENT PLAN — pre-registered — **REVISION 5**

**Revision:** 5
**Written:** 2026-08-18
**Supersedes:** `EXPERIMENT_PLAN_R4.md` (revision 4),
...
→ rev 4 `738601db…cc9d` → **rev 5**

Revisions 1–4 remain in the repository **byte-identical**. All four hashes were
re-verified immediately before this revision was written and again after it was
committed.

**Authority:** revision 4 §5.6 rule 4 — *"If the plan is superseded wholesale, a
...
**Why there is a revision 5.** S3b implemented the two scorers that decide **C2**
— the anchor-tracking scorer (§2.4.7, §2.8) and the generalised signature scorer
(§2.5, §2.6) — and ran them against 22 synthetic fixture assertions. Both work:
**C15 confirmed that with free-prose forced `indeterminate`, `repair artifact` and
`format tax` still separate.** But the implementation found **18 defects, five
blocking** (`audit/S3B_SCORER_DEFECTS.md`), two of them sitting directly on C2's
path:

- **the pre-repair tracking aggregate had no registered cell field**, though both
  instrument rivals hinge on it (S3B-12);
- **the per-field chance rule, implemented symmetrically as briefed, made
  `quantisation`, `decoding` and `genuine prior` unmatchable on the tracking
  column** (S3B-11).

Revision 5 folds in all 18.
...
read. **That ordering has now caught defects twice** — 25 at S3a against
revision 3, 18 at S3b against revision 4 — and it is registered rather than
optional.
...
**Section numbering is preserved** from revisions 1–4.

---

## Amendment record — revision 4 → revision 5

| ID | Amendment | Sections |
|---|---|---|
| **R5-1** | **One-sided per-field chance rule** registered for anchor tracking. The flat 0.50 bar and the symmetric per-field rule are both **rejected**, with S3b's disagreement table as the evidence. Fixes S3B-10, S3B-11, S3B-16. | §2.6 |
| **R5-2** | Anchor tracking runs at **all three models** (12 cells, 240·B_tracking generations). **X5 stays ONE contrast; FAMILY_SIZE stays 17; ALPHA stays 0.0029411764705882353.** Fixes S3B-17. | §2.2, §2.8, §5.2 |
| **R5-3** | **`tracks_{first,exemplar}_pre_mean` and `_ci95` registered at cell level.** Without these the column could not be scored from the results file at all. Fixes S3B-12. | §5.5 |
| **R5-4** | **Field-collapse threshold registered** (normalised per-field entropy < 0.15) and **both tracking denominators stated** — `tracks_first` over all collapsed fields, `tracks_exemplar` over the three the exemplar names. Fixes S3B-01, S3B-02. | §2.4.4, §2.4.7 |
| **R5-5** | The five tracking rows written as **explicit predicates over the 2×2 grid**; every rival predicts on **both** sub-quantities; no `and/or`. Fixes S3B-05. | §2.5 |
| **R5-6** | Indeterminacy may create winners — a threshold scaling with *n_s* must behave that way. Instead the verdict is flagged **`contingent_on_indeterminacy`** when the winner's matched count is below ceil(0.75·6) = 5. Fixes S3B-07. | §2.6 |
| **R5-7** | The **known limitation** recorded plainly: the free-prose and schema-pre columns cannot produce a mismatch, so the five-rival set is not falsifiable in the strong sense on half its columns. Fixes S3B-09. | §2.5 |
| **R5-8** | **`B_tracking = max(16, the value a tracking-CI-width pilot confirms at S3)`**, floor 16, on the same footing as R and B_batch. Raising it is compliance. Fixes S3B-18. | §2.8, §3.5 |
| **R5-9** | Block D coverage gaps closed: `label_tracking` carries the grid; per-field modal values, the exemplar value map, the per-field chance rates and ΔD all get registered fields. Fixes S3B-13, S3B-14, S3B-15. | §5.5 |
| **R5-10** | Remaining material and minor defects resolved: bootstrap unit, the `recovers` demotion, the open CI boundary, the prose/rule mismatch. Fixes S3B-03, S3B-04, S3B-06. | §2.4.5, §2.4.7, §2.6 |
| **R5-11** | **Full discreteness table re-run for all 17 confirmatory tests. Every floor clears.** | §5.2 |

**Family check.** R5-2 adds cells and models but **no confirmatory test**: column
classification is descriptive by CI position, not by hypothesis test, so per-model
scoring needs no per-model contrast. R5-1, R5-3 … R5-11 add none either.
**FAMILY_SIZE = 17; ALPHA = 0.05/17 = 0.0029411764705882353 — unchanged from
revision 4.**

**The S3b defects are dispositioned in §9; the S3a defects revision 4 closed are
carried in §10.** Section numbering is otherwise preserved from revisions 1–4;
§9 is new and §10 is revision 4's §9 renumbered.

*Revision 4's own amendment record (R4-1 … R4-5) is not repeated here; it is in
`EXPERIMENT_PLAN_R4.md`, which remains byte-identical, and its dispositions are
carried forward in §10.*
...
### The tracking predicates — REGISTERED (R5-5, fixes S3B-05)

Revision 4 stated the tracking cells in prose — *"post-repair modal tracks
first-enumerated; pre-repair does not"*, *"pre-repair modal tracks
first-enumerated **and/or** exemplar"* — while §2.6's label vocabulary defined a
label for **one** proportion. The observation is a **2×2 grid**; nothing said how
a grid matches a sentence. **Every rival now predicts on both sub-quantities at
both stages, and the `and/or` is gone.**

The grid, with each entry taking `tracks` / `no tracking` / `indeterminate` from
§2.6:

```
                 tracks_first     tracks_exemplar
    pre-repair   G[pre,first]     G[pre,exemplar]
    post-repair  G[post,first]    G[post,exemplar]
```

| ID | Rival | Predicate | Why |
|---|---|---|---|
| **R-A** | repair artifact | `G[post,first] = tracks` **AND** `G[post,exemplar] = no tracking` **AND** `G[pre,first] = no tracking` **AND** `G[pre,exemplar] = no tracking` | `sanitize_config` coerces and fills to `valid_vals[0]` — literally the first-enumerated value — so reversing the enumeration reverses the repair target. The exemplar is a *prompt* object and the repair stage never sees it, so a repair artifact must NOT track the exemplar. The model's own pre-repair output is untouched. |
| **F-T** | format tax | ( `G[pre,first] = tracks` **OR** `G[pre,exemplar] = tracks` ) **AND** `G[post,first] = G[pre,first]` **AND** `G[post,exemplar] = G[pre,exemplar]` | The prompt's presentation drives the model's own choice, so the effect is present **before** any repair, on whichever of the two the prompt makes salient; and repair, which only coerces illegal values, leaves a legal choice unchanged. **The disjunction is necessary, not vague:** in a dissociable cell the first-enumerated and exemplar values differ, so a single modal value cannot equal both, and a conjunction would be logically unsatisfiable. |
| **NONE** | quantisation, decoding, genuine prior | all four grid entries `= no tracking` | Precision, temperature and the model's own weights are all invariant to how the harness lists the options. |

**Evaluation rule.** A predicate is `indeterminate` **only if a grid entry it
reads is indeterminate**; entries it does not read cannot make it so. R-A reads
three entries, F-T and NONE read all four.

**Why F-T is a disjunction and R-A is not — registered, found by implementation.**
Operator decision 4 asked every rival to predict on *both* sub-quantities. Read as
a **conjunction** that is **logically impossible for F-T**: in a dissociable cell
the first-enumerated and exemplar values differ by construction, and a single
modal value cannot equal both, so `G[pre,first] = tracks ∧ G[pre,exemplar] =
tracks` can never hold. The decision is honoured by having F-T state a value for
**every** grid entry — a disjunction on the pre pair plus an equality constraint
tying post to pre — rather than by forcing a conjunction that no data could
satisfy. R-A's conjunction *is* satisfiable, because "no tracking" on two entries
and "tracks" on a third is a consistent world.

The three predicates remain **mutually exclusive**: R-A requires both pre entries
`no tracking`, F-T requires at least one of them `tracks`, NONE requires all four
`no tracking`.
...
### B_tracking — a floor plus a procedure (R5-8, fixes S3B-18)

> **`B_tracking = max(16, the value the S3 tracking-CI-width pilot confirms)`.**
> **Floor: 16.**

S3b measured the BCa interval on a proportion at *n* = 16 batches at roughly
±0.25 at mid-range. Under the rejected flat bar that made the column
`indeterminate` across most of the plausible range; under the registered
one-sided rule (§2.6) it is determinate across the whole sweep, **but the margin
at rates just above chance is thin**, and the column carries C2.

The S3 pilot is a **pure resampling exercise on synthetic proportions** — no
model, no generation — that returns the smallest batch count whose 95% interval
half-width at a true rate of 0.40 is below 0.15, i.e. tight enough to separate
0.40 from the 0.264 chance rate. **If that value exceeds 16, `B_tracking` takes
it, and doing so is COMPLIANCE with this plan, not a deviation**, on exactly the
footing §3.4 gives R. A value below 16 does not license going below 16.
`B_tracking` is fixed once, before the first anchor run, and recorded as
`B_tracking`, `B_tracking_floor` and `B_tracking_source`.

Note that R5-2's extension to three models multiplies **cells**, not *n* per cell,
and therefore does **not** address the width problem on its own. `B_tracking` is
the lever that does.
...
- Paired sign-flip permutation, paired on **(batch index, exemplar level)**:
  `B_tracking` batches × 2 exemplars = **2·B_tracking paired differences**
  (32 at the floor).
...
| E1 anchor tracking — 12 cells × `B_tracking` × 20 | **240 · B_tracking** |
| **E1 total** | **9,600 + 240 · B_tracking** |

At the `B_tracking` floor of 16: **13,440**.
...
| E1 anchor tracking — 12 × `B_tracking` × 20 | **240 · B_tracking** |
| E2 primary — 4 arms × R × 20 × 3 tasks | 240·R |
| E2 conditional replicate — 4 × R × 20 × 1 task | 80·R |
| E3 | 0 |
| **Total** | **9,600 + 240·B_tracking + 320·R generations, zero training runs** |

At both floors (`B_tracking` = 16, R = 20): **19,840**. At `B_tracking` = 20,
R = 20: 20,800. At `B_tracking` = 24, R = 30: 24,960.

*(rev 1: 9,400 · rev 2: 12,400 · rev 3: 16,000 · rev 4: 17,280 · rev 5: 19,840,
all at the floors.)*

**The budget is a function of two quantities the S3 pilots set, and this plan caps
neither.** Roughly 1,280 of the anchor generations at the floor are frontier API
calls, which is the only line with a per-call cost.
...
| A21 | E1 | `tracks_first` and `tracks_exemplar` proportions with BCa intervals, **per cell × stage (all four aggregates)**; modal-tie counts | descriptive |
...
| **A26** | E1 | **`contingent_on_indeterminacy` flag, `n_indeterminate`, `would_win_at_n_s_6`** *(new, R5-6)* | **classification** |

**Confirmatory tests: X1–X4 across 3 models = 12, plus X5 = 1, plus Y1–Y4 = 4.
Total 17 — unchanged from revision 4.** A1–A26 carry no p-value.
...
**R5-11 — the full table at ALPHA = 0.0029411764705882353, all 17 tests.**
...
| 13 | **X5 enumeration order** | anchor | `monte_carlo` | N = 100,000 | 9.999900e-06 | 0.340% | **PASS** |
| 14 | Y1 uncurated vs zero-shot | — | `monte_carlo` | N = 100,000 | 9.999900e-06 | 0.340% | **PASS** |
| 15 | Y2 curated vs zero-shot | — | `monte_carlo` | N = 100,000 | 9.999900e-06 | 0.340% | **PASS** |
| 16 | Y3 archive vs zero-shot | — | `monte_carlo` | N = 100,000 | 9.999900e-06 | 0.340% | **PASS** |
| 17 | Y4 uncurated vs curated | — | `monte_carlo` | N = 100,000 | 9.999900e-06 | 0.340% | **PASS** |
...
| X5 if enumerated exactly | 2³² = 4,294,967,296 | > 10⁷ | `monte_carlo` | *(4.66e-10, not used)* |
| Y1–Y4 if enumerated exactly | C(40,20) = 137,846,528,820 | > 10⁷ | `monte_carlo` | *(1.45e-11, not used)* |

**If `B_tracking` rises above 16 (§2.8), X5's paired count rises with it and its
Monte-Carlo floor is unchanged at 9.999900e-06** — the floor depends on N, not on
the pair count, so no re-check is needed on that axis.
...
The same applies to A16–A26: intervals, flags, grids and gate records, never
p-values.
...
**`schema_version` is `1.4.0`** (rev 1: 1.0.0; rev 2: 1.1.0; rev 3: 1.2.0;
rev 4: 1.3.0). All changes are additive — every earlier key keeps its meaning.
...
```json
{
  ...
  "plan_supersedes_sha256": "738601db1d55e81010a62ec1e1259f82e6466f7e8db02f0ec3de4ed15d80cc9d",
  "plan_chain_sha256": [
  ...
  "b_tracking": {"value": 16, "floor": 16, "source": "S3 tracking-CI-width pilot"},
  ...
  "cells": [
    {
      ...
      "label_tracking_grid": {"pre_first": "", "pre_exemplar": "",
                              "post_first": "", "post_exemplar": ""},
...
- **reducing `B_tracking` below `max(16, the S3 pilot value)`** (R5-8). The floor
  of 16 binds independently, exactly as R's floor of 20 does;
