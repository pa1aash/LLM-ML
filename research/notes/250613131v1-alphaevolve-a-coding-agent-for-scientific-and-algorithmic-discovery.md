...
Evaluation.
Since AlphaEvolve tackles problems with machine-gradeable solutions, the user
...
API.
To support evolving multiple components across a codebase, AlphaEvolve exposes an
...
AlphaEvolve: A coding agent for scientific and algorithmic discovery
Output format.
When AlphaEvolve asks an LLM to modify existing code, especially within
...
Models used.
AlphaEvolve employs an ensemble of large language models. Specifically, we
...
Multiple scores.
AlphaEvolve allows for optimizing multiple user-provided scores, i.e.,
...
matrix and an 𝑛× 𝑝matrix, i.e. the number of scalar multiplications required to compute this
...
3
def
_get_optimizer (self) -> optax. GradientTransformation :
4
""" Returns
optimizer ."""
5 -
...
10
def
_get_init_fn (self) -> jax.nn. initializers . Initializer :
11
""" Returns
initializer
function ."""
...
self.hypers.init_scale , jnp.complex64 )
13 +
# Initialize
with a smaller
scale to
...
16 +
return
initializers .normal (0 + 1j * 0, scale * 0.2, jnp.complex64 )
17
18 @@
-80,6 +85 ,66 @@
19
...
101 +
def
hallucinate (x, hallucination_rng ):
102 +
...
error.
116 -
rec_loss = l2_loss_complex (self. target_tensor [None , ...] ,
rec_tensor )
117 +
rec_loss = l2_loss_complex ( noisy_target_tensor [None , ...] ,
rec_tensor )
118
119
# We must
...
loss.
148 +
cycle_length = self.config. training_steps
// 4
# Number
of steps
per
...
165 +
large_value_penalty
+= jnp.mean(jnp.abs(factor) ** 2)
166 +
large_value_penalty
/= len( decomposition )
167 +
total_loss
+= self.hypers. large_value_penalty_weight * large_value_penalty
168 +
169 +
return
jnp.real(total_loss)
...
172
def
l2_loss_complex (x: jnp.ndarray , y: jnp.ndarray) -> jnp.ndarray:
173
""" Elementwise
L2
loss
...
hyper.zipit ([
176 -
hyper.uniform('init_scale ', hyper.interval (0.2 ,
1.5)),
177 -
hyper.uniform(' learning_rate ', hyper.interval (0.05 ,
0.3)),
178 +
hyper.uniform('init_scale ', hyper.interval (0.1 ,
1.0)),
179 +
hyper.uniform(' learning_rate ', hyper.interval (0.01 ,
0.2)),
180 +
hyper.uniform(' discretization_weight ', hyper.interval (0.0 ,
0.1)),
181 +
hyper.uniform(' hallucination_prob ', hyper.interval (0.0 ,
0.2)),
182 +
hyper.uniform(' hallucination_scale ', hyper.interval (0.0 ,
0.2)),
183 +
hyper.uniform('noise_std ', hyper.interval (0.0 ,
0.01)),
184 +
hyper.uniform(' target_noise_std ', hyper.interval (0.0 ,
0.01)),
185 +
hyper.uniform(' weight_decay ', hyper.interval (0.00001 ,
0.001)),
186 +
hyper.uniform('clip_min ', hyper.interval (0.0 ,
0.5)),
187 +
hyper.uniform('clip_max ', hyper.interval (1.0 ,
3.0)),
188 +
...
exploration .
190 +
hyper.uniform(' grad_noise_std ', hyper.interval (0.0 ,
0.001)),
191 +
hyper.uniform(' half_int_start ', hyper.interval (0.0 ,
1.0)),
192
])
193
...
-
return
initializers .normal (0.0 , self.hypers.init_scale , jnp.
complex64)
13
+
# Initialize
...
AlphaEvolve extends a long tradition of research on evolutionary
...
Author information
These authors contributed equally: Alexander Novikov, Ngân V˜u, Marvin Eisenberger, Emilien
...
N.V. and P.-
S.H. oversaw the practical applications.
E.D. and M.E. implemented the first benchmark
problem used for iterating on AlphaEvolve, with input from F.J.R.R. and M.B.
A.N. and M.E.
developed the final version of AlphaEvolve, with contributions from S.S., P.-S.H., and input
from M.B., E.D., A.Z.W. and N.V.
A.N., S.S., P.-S.H. and M.E. maintained the infrastructure
underlying AlphaEvolve.
M.E. and E.D. used AlphaEvolve to discover new algorithms for
matrix multiplication, with input from F.J.R.R.
A.Z.W. worked on the applications to open
mathematical problems, with help from A.M., M.E., and A.N.
A.N. contributed to the
Borg scheduling application.
P.-S.H. and N.V. worked on the application to Gemini kernel
engineering.
P.-S.H. and A.N. contributed to the TPU circuit design application.
B.K. and
S.S. worked on applying AlphaEvolve to directly optimize compiler-generated code.
M.E.
performed the ablation experiments.
M.B., A.N., M.E., S.S. and P.-S. H. performed the
22

AlphaEvolve: A coding agent for scientific and algorithmic discovery
majority of code reviews.
M.B., E.D., S.C., N.V., A.Z.W., F.J.R.R., M.E., A.N., B.K., S.S., A.M.,
and M.P.K. wrote the paper, with input from A.S., P.-S.H and P.K. N.V., E.D., M.E., S.C., A.N.,
and A.Z.W. created the figures.
F.J.R.R., A.M., and A.Z.W. assembled the accompanying
Google Colab.
S.N., A.D. and P.K. advised and enabled multiple strands of this work.
M.B.,
A.N., N.V. and G.H. coordinated the team.
P.K. supervised and coordinated the research
program.
...
[37] X. Gu and M. Krenn.
Interesting scientific idea generation using knowledge
...
Through Large Models, pages 331–366.
...
We provide the best ranks obtained by AlphaEvolve in Table 3. Overall,
...
12 -
return
initializers .normal (0.0 , self.hypers.init_scale , jnp.complex64)
13 +
# Initialize
with a smaller
scale to
...
For any function 𝑓: ℝ→ℝ, define the autoconvolution of 𝑓as
𝑓∗𝑓(𝑡) B
∫
ℝ
𝑓(𝑡−𝑥) 𝑓(𝑥) 𝑑𝑥.
...
𝑓(𝑥) 𝑑𝑥
2
for any function 𝑓: ℝ→ℝ. Clearly 𝐶3 ≤𝐶1, since we now allow 𝑓to take positive and
...
𝐴( 𝑓)𝐴( ˆ𝑓) ≥𝐶4
for all even 𝑓with max( 𝑓(0), ˆ𝑓(0)) < 0. It is known [33] that
0.2025 ≤𝐶4 ≤0.3523.
...
bound 𝐶4 ≤𝐴( 𝑓)𝐴( ˆ𝑓). Following the approach in [33], the test function is sought in the form
𝑓(𝑥) = 𝑃(𝑥)𝑒−𝜋𝑥2, where 𝑃(𝑥) is an even polynomial constructed as a linear combination of
Hermite polynomials 𝐻4𝑘(𝑥). This form is particularly useful because the Fourier transform of
𝐻𝑛(𝑥)𝑒−𝜋𝑥2 is 𝑖𝑛𝐻𝑛(𝜉)𝑒−𝜋𝜉2. For an even polynomial 𝑃(𝑥) = Í 𝑐4𝑘𝐻4𝑘(𝑥), the Fourier transform
of 𝑓(𝑥) is ˆ𝑓(𝜉) = Í 𝑐4𝑘𝑖4𝑘𝐻4𝑘(𝜉)𝑒−𝜋𝜉2 = (Í 𝑐4𝑘𝐻4𝑘(𝜉))𝑒−𝜋𝜉2 = 𝑃(𝜉)𝑒−𝜋𝜉2. Thus, 𝐴( 𝑓) is related
to the largest positive root of 𝑃(𝑥), and 𝐴( ˆ𝑓) is related to the largest positive root of 𝑃(𝜉).
Specifically, if 𝑃(𝑥) ≥0 for large |𝑥|, 𝐴( 𝑓) is the largest positive root of 𝑃(𝑥), and 𝐴( ˆ𝑓) is the
...
𝑐1𝐻4(𝑥)+𝑐2𝐻8(𝑥)+. . . such that 𝑃(𝑥) satisfies certain constraints (related to 𝑓(0) < 0, ˆ𝑓(0) < 0
...
positive root 𝑟max of 𝑃(𝑥) is then the largest positive root of 𝑃(𝑥)/𝑥2. The upper bound on 𝐶4
...
∫
𝑓= 1, where we
extend 𝑓, 𝑔by zero outside of [−1, 1]. This constant controls the asymptotics of the Minimum
...
on [0, 2] with values in [0, 1] and satisfying
∫2
0 ℎ(𝑥)𝑑𝑥= 1 of
max
𝑘
...
large finite sets of integers 𝐴, 𝐵with |𝐴+ 𝐵| ≪|𝐴| and |𝐴−𝐵| ≫|𝐴+ 𝐵|𝐶6. (Here 𝐴+ 𝐵=
...
sets 𝐴, 𝐵(for sufficiently large sets 𝐴, 𝐵). The notation 𝑋≫𝑌means that 𝑋≥𝐶′𝑌for some
...
log(2 max(𝑈) + 1)
(3)
for any finite set 𝑈of non-negative integers containing zero satisfying |𝑈−𝑈| ≤2 max(𝑈) + 1.
...
For any 𝑛and 𝑑, the goal of this problem is to find 𝑛points in the 𝑑-dimensional space so as
...
√
12.890 [29]. (In this reference, instead of the ratio itself, the
...
Lemma 1. Let 𝐶⊂ℝ𝑑be a set of points satisfying 0 ∉𝐶and
...
∥𝑥∥: 𝑥∈𝐶
o
form a valid kissing configuration in dimension 𝑑. In
particular, the kissing number in dimension 𝑑is at least |𝐶|.
Proof. For any 𝑥≠𝑦∈𝐶, the inequality ∥𝑥−𝑦∥2 ≥max{∥𝑥∥2, ∥𝑦∥2} implies
2⟨𝑥, 𝑦⟩≤∥𝑥∥2 + ∥𝑦∥2 −max{∥𝑥∥2, ∥𝑦∥2} = min{∥𝑥∥2, ∥𝑦∥2} ≤∥𝑥∥· ∥𝑦∥,
(4)
where the last inequality holds because the minimum of two positive numbers is less than or
equal to their geometric mean. The points
n
2𝑥
∥𝑥∥: 𝑥∈𝐶
o
...
After simplifying, this is equivalent to 2⟨𝑥, 𝑦⟩≤∥𝑥∥· ∥𝑦∥, which we have proved in (4). Thus
unit spheres centred at
n
2𝑥
∥𝑥∥: 𝑥∈𝐶
o
form a valid kissing configuration in dimension 𝑑, as
required.
□
