---
title: '[2206.08896v1] Evolution through Large Models'
id: 220608896v1-evolution-through-large-models
tags:
- llm-nas-feedback-positioning-7125b1
- llm-guided-evolution
- genetic-programming
created: '2026-08-16T15:45:18.542624Z'
updated: '2026-08-16T15:46:25.369200Z'
source: https://arxiv.org/abs/2206.08896v1
source_domain: arxiv.org
fetched_at: '2026-08-16T15:45:18.542027Z'
...
content_type: paper
deprecated: false
summary: 'Lehman, Gordon, Jain, Ndousse, Yeh, Stanley (2022, OpenAI/Uber AI lineage).
  Introduces Evolution through Large Models (ELM): using code-generating LLMs as mutation
  operators inside genetic programming / MAP-Elites, rather than hand-coded random
  mutations. Main experiment: ELM+MAP-Elites generates hundreds of thousands of functional
  Python programs producing working ambulating robots in the Sodarace domain the base
  LLM never saw in pretraining, and these outputs are then used to bootstrap a new
  conditional model that outputs a fitting walker for a given terrain (self-distillation
  from search products). Relevant as an early, influential LLM-guided evolutionary-search
  precedent where the LLM''s role is a smart mutation/proposal operator rather than
  a one-shot zero-feedback proposer, useful as a comparison point for whether feedback/iteration
  helps or hurts LLM-guided search in adjacent (non-NAS) program-search domains.'
---

[2206.08896v1] Evolution through Large Models
...
,
Kenneth O. Stanley
View a PDF of the paper titled Evolution through Large Models, by Joel Lehman and Jonathan Gordon and Shawn Jain and Kamal Ndousse and Cathy Yeh and Kenneth O. Stanley
View PDF
HTML (experimental)
Abstract:
...
[cs.NE]
for this version)
https://doi.org/10.48550/arXiv.2206.08896
Focus to learn more
...
[v1]
Fri, 17 Jun 2022 17:07:04 UTC (1,901 KB)
Full-text links:
Access Paper:
View a PDF of the paper titled Evolution through Large Models, by Joel Lehman and Jonathan Gordon and Shawn Jain and Kamal Ndousse and Cathy Yeh and Kenneth O. Stanley
View PDF
...
surprises.
Yet are they ultimately competing paradigms, or rather are they
complementary?
In this paper we explore the latter possibility, of consider-
...
progressed at an impressive pace in recent years.
The main idea (in auto-
...
large-scale data. A further development are diﬀmodels that are trained on
diﬀs from GitHub [31]. A diﬀis an incremental change to a ﬁle that is com-
mitted to a version control system such as GitHub, accompanied by a commit
message describing the intent of the change. In this way, diﬀmodels are trained
how, given a piece of code and any potential commit message, to suggest an
informed change. Through the lens of evolutionary algorithms, such diﬀmod-
...
evolution). Both diﬀmodels and their capacity for self-adaptation are explored
...
diﬀmodels when given the right kinds of prompts (see Section 3.1).
2.3
Open-endedness
...
exposing them to extensive programming examples [9, 10]. A diﬀmodel [31]
can similarly be autoregressively trained on a collection of code diﬀs (e.g. from
GitHub). Each diﬀtargets a single ﬁle, where the ﬁle and diﬀare short enough
to ﬁt into the context of the LLM. The model is trained to predict the diﬀ
(formatted, for example, in uniﬁed diﬀformat [44]) from the concatenation of
the ﬁle and the commit message, where the loss includes only the tokens that
make up the diﬀ, thereby encouraging the model to predict the diﬀbut not to
memorize the ﬁle and commit message. In other words, the model learns to
predict plausible changes to code from examples of changes made to code by
human programmers. It is important to note that the idea of diﬀmodels (or
their initial training) [31] is not a contribution of this paper, but diﬀmodels are
...
As a simple experiment to highlight diﬀmodels’ ability to intelligently mod-
...
diﬀmodel. The hypothesis is that an intelligent perturbation operator will be
...
[3]) serves as a representative test-bed.
Note that a correct implementation
of 4-Parity returns the sum of the four input bits, modulo two.
Up to ﬁve
...
In contrast, the diﬀoperator is able to ﬁx all ﬁve bugs, and its performance
...
of through explicit training (as in the diﬀmodel). That is, one could design
prompts for a model trained on code (like Codex [9] or GPT-6-J [47]).
To
...
engineering and edit mode to the diﬀmodel. Figure 2 shows how models from
the API outperform the diﬀmodel used in the paper. Further experimental
details can be found in Appendix A.
3.2
The Evolutionary Algorithm and Implications for Open-
Endedness
...
Figure 1: Comparing diﬀmutation to GP mutation in ﬁxing 4-Parity
...
no solutions when there are ﬁve bugs). In contrast, diﬀmutation degrades only
...
strong open-endedness.
This gap remains despite that recent work in open-
8
...
edit mode and prompt-engineering approaches outperform the diﬀmodel applied
...
a hideout.
Yet despite such algorithms clearly demonstrating the capability
...
diﬀmodel and evaluated. The new candidate solution is assigned its niche from
...
a diﬀmodel. Each replica generates a batch of diﬀs that are applied to the
...
In particular, the
pre-trained diﬀmodel can be trained further (which is called ﬁne-tuning) with
accepted diﬀs (by MAP-Elites) from initial iterations or runs of ELM. That way,
the diﬀoperator updates to understand better the kinds of modiﬁcations that
...
role of QD (i.e. MAP-Elites in this paper). Combined with the diﬀfunction, the
...
Preliminary experiments showed that the diﬀmodel’s initial performance
...
the solution within that niche is perturbed by the diﬀmodel and evaluated. The
...
Figure 7: The three seed solutions.
From top to bottom: CPPN seed,
...
tion). A 300M parameter pretrained diﬀmodel [31] served as the perturbation
...
Figure 8: Amount of niches ﬁlled across seeds.
The ﬁgure shows the
...
seed robustness).
Fine-tuning the diﬀmodel on accepted diﬀs from an initial series of runs
greatly increased performance (Figure 11); while Sodarace-generating programs
are out-of-distribution for the pretrained diﬀmodel (applying a Python encoding
to this domain is a novel enterprise), ﬁne-tuning eﬀectively aligns the diﬀmodel
...
their higher performance, the output of runs applying the ﬁne-tuned diﬀmodel
...
experiments showed diminishing returns.
Future work could explore how to
...
diﬀs.
The seeds and ﬁne-tuned diﬀmodel also qualitatively impact the kinds of
...
6
Pipeline Stage 2: Language Model Training
...
that ﬁne-tuning the diﬀmodel has a signiﬁcant impact on attained QD score,
...
ﬁne-tuned diﬀmodel (3 runs for each of the Square, CPPN-Fixed, and CPPN-
Mutable seeds). In total, the 50% cut-oﬀthreshold dataset consisted of 280K
examples, and the 80% cut-oﬀthreshold dataset contained a subset of 95K of
those examples.
...
(b) QD Score
(c) DiﬀQuality
Figure 11: The impact of ﬁne-tuning the diﬀmodel on the performance
of ELM. For both the pretrained diﬀmodel and the ﬁne-tuned one, shown are
...
ﬁne-tuning the diﬀmodel improves performance of the evolutionary process
...
is shown.
Model sizes above 85M may not better-ﬁt the data, and random
...
of samples from it and evaluating them in the same way.
These results are
...
the 80% cutoﬀdataset, reﬂecting the higher average QD of that dataset.
23
...
in Stage 2, nor was it used to ﬁne-tune the diﬀmodel in Stage 1; thus the ability
...
7
Pipeline Stage 3: Conditional RL
...
bution over a sequence of tokens x = (x1, . . . , xn) as pθ(x) = Qn
i=1 pθ(xi|x<i).
...
bution support.
To reduce the viability of such an outcome and simulate a
...
Discrete Codes.
The terrain distribution has a discrete and ﬁnite support.
...
ResNets.
An alternative parametrization is visual representations of the ter-
...
cutoﬀthresholds in {50%, 80%}. Recall that we use the cutoﬀthreshold to con-
trol the tradeoﬀbetween data quality and quantity, such that higher thresholds
...
call that there is one model for both cutoﬀthresholds), and the best-performing
29
...
that are capable of locomoting on particular terrains.
Moreover, Figures 18
...
of cutoﬀthreshold. A particularly interesting question is whether RL is able
...
Models trained with a cutoﬀof 80% tend to achieve slightly better per-
formance, and proved more stable during training, though the diﬀerences are
not signiﬁcant. This result implies that the tradeoﬀbetween data quality and
...
50% cutoﬀthreshold, the Stage 3 (discrete) model improves upon all stages,
...
Results are detailed when training the LM using a dataset with a cutoﬀof (a)
...
For the 80% cutoﬀthreshold, the pipeline achieves comparable performance to
that of the dataset, while improving performance for the 50% cutoﬀthreshold.
33
...
a reality.
The MAP-Elites algorithm combined with ELM easily bootstraps
datasets of hundreds of thousands of examples in a completely foreign domain
(to the initial LLM) from initial human-written seeds.
...
Acknowledgments
Thank you to JeﬀClune for substantive insightful feedback and thoughtful
discussion about the project and this paper.
Thanks also to Glenn Powell
...
[22] Joel Lehman, Jay Chen, JeﬀClune, and Kenneth O Stanley. Safe mu-
...
[37] Christopher Stanton and JeﬀClune. Curiosity search: producing gener-
...
[44] Detailed description of uniﬁed format.
https://www.gnu.org/software/
diﬀutils/manual/html node/Detailed-Uniﬁed.html.
[45] Open AI blogpost: New GPT-3 capabilities: Edit and insert.
https://
openai.com/blog/gpt-3-edit-insert/, 2022.
[46] Long Ouyang, JeﬀWu, Xu Jiang, Diogo Almeida, Carroll L Wainwright,
...
[47] Ben Wang and Aran Komatsuzaki.
GPT-J-6B: A 6 Billion Param-
...
[50] Justin K Pugh, Lisa B. Soros, and Kenneth O. Stanley.
Quality di-
versity:
A new frontier for evolutionary computation.
3(40), 2016.
ISSN 2296-9144. URL http://www.frontiersin.org/evolutionary-robotics/
10.3389/frobt.2016.00040/abstract.
[51] Jean-Baptiste Mouret and JeﬀClune. Illuminating search spaces by map-
ping elites. ArXiv e-prints, abs/1504.04909, 2015. URL http://arxiv.org/
abs/1504.04909.
[52] Rui Wang, Joel Lehman, JeﬀClune, and Kenneth O. Stanley.
Poet:
...
[53] Jonathan C. Brant and Kenneth O. Stanley.
...
Bob McGrew, and Igor Mordatch.
Emergent tool use from multi-agent
autocurricula. arXiv preprint arXiv:1909.07528, 2019.
[55] Antoine Cully, JeﬀClune, Danesh Tarapore, and Jean-Baptiste Mouret.
...
[60] Kenneth O. Stanley and Risto Miikkulainen.
A taxonomy for artiﬁcial
...
[62] Josh C. Bongard and Rolf Pfeifer.
Repeated structure and dissociation
...
35–43, 1999.
[64] Richard S. Sutton and Andrew G. Barto.
...
[70] Nisan Stiennon, Long Ouyang, JeﬀWu, Daniel M Ziegler, Ryan Lowe,
...
message for the diﬀmodels for these tasks is “Fixed bugs.”
Note that GP
...
A.1
Python Source for Benchmark Tasks
A.1.1
4-Parity
1 #!/usr/bin/python3
...
ﬁrst “b1” is changed to “c1”, then “b2” is changed to “c2”, etc.).
For the
...
Figure 20: Comparing diﬀmutation to GP mutation at ﬁxing bugs in
...
the second bug is added, while diﬀmutation is unaﬀected. The conclusion is
...
A.2
Comparing GP and DiﬀMutation
Performance plots that compare GP to diﬀmutation for the 4-Parity task are in
...
bugs, and degrades in a diﬀerent pattern (here, the performance of diﬀmutation
is unaﬀected by the introduction of the second bug).
A.3
Comparing API-based Mutations
Performance plots that compare diﬀmutation to mutations possible through
...
outperform the 300M diﬀmodel applied in this paper’s experiments. The con-
...
that there are complex interactions between the kind of seed that kicks oﬀELM
...
provide good jumping-oﬀpoints to adapt to novel terrains in Stage 3. When
...
diﬀmodel. The conclusion is that QD score does not entirely capture what
enables generalization and adaptation to novel terrains.
Understanding this
...
evolved), the pretrained diﬀmodel behaves similarly to the Radial seed (it cre-
ates inventions with high quantitative performance but which exploit chaotic
dynamics). However, when the diﬀmodel is ﬁne-tuned on the products of the
...
This approach strikes a diﬀerent trade-oﬀbetween quantity and quality of
...
all reasonably-high quality solutions.
The ﬁnal map approach, on the other
...
to be too small to serve as a reliable jumping-oﬀpoint for further RL.
F
Source Code for Completion Targets
...
G
Source Code for Selected Stage 1 Sodaracers
G.1
Blob (from CPPN Seed)
...
together springs and masses to create functional Sodarace inventions).
This
