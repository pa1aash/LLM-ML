# EXPERIMENT PLAN — pre-registered — **REVISION 2**
...
Revision 1 remains in the repository **byte-identical**. It is not edited, not
deleted, and not amended in place. This file is the governing document from the
moment its own hash is recorded in `PREREGISTRATION.md`; revision 1 is the
historical record of what was registered first.

**Authority for writing a successor rather than editing:** revision 1 §5.6
rule 4 — *"This plan is not edited after its hash is taken… If the plan is
superseded wholesale, a numbered revision is written as a new file, hashed
separately, and cites the prior hash it supersedes; the original file remains in
the repository unmodified."* That rule is carried forward unchanged as §5.6
rule 4 below and governs any revision 3.

**Status at time of writing:** no data collected. Re-verified immediately before
this revision was hashed — see `PREREGISTRATION.md`. Nothing in revision 1 or
revision 2 has been run: no model served, no generation produced, no benchmark
queried, no analysis script written, no hardware provisioned, no ML dependency
installed.

**Section numbering is preserved from revision 1.** Every existing cross-reference
elsewhere in the repository (`GATES.md`, `STATE.md`, `VENUE.md`,
`audit/OPEN_ACTIONS.md`) resolves to the same content it did before. Amendments
extend sections; they do not renumber them. §8 is new and appends.
...
|---|---|---|
| **A1** | E1 batches per cell 5 → **10** (n = 200 per cell, **6,000** generations). Shared seed vector extended to ten. | §2.3, §3.5, §5.5 |
| **A2** | Add a **within-cell bootstrap CI on D**, and on ΔD for the two change columns. Secondary resolution check; the paired batch-index contrast remains primary. | §2.4.5, §5.1, §5.5 |
| **A3** | Add a **parse-free surface diversity measure S** on raw generated text. Secondary only; never substituted into D, never confirmatory. Bounds the free-prose parse-selection effect. | §2.4.6, §2.6, §2.7, §5.1, §5.5 |
| **A4** | E2 **retains all three tasks** — CIFAR-100 primary, CIFAR-10 and ImageNet16-120 secondary — with the retention reasoning recorded. | §3.1 |
| **A5** | The calendar consideration is recorded as **CONSIDERED AND REJECTED**. Scope is set by merit; the venue follows the artifact. | §8 (new) |
| **A6** | Confirmatory family **unchanged: FAMILY_SIZE = 16, ALPHA = 0.003125**. No amendment adds a confirmatory test. | §5.2 |

**A6 verification, performed before this revision was written.** Each amendment
was checked against the family definition in §5.2:

| Amendment | Adds a confirmatory test? | Why not |
|---|---|---|
| A1 | No | More batches per cell changes the *precision* of X1–X4, not their number. Identical four contrasts, identical three models. |
| A2 | No | The bootstrap emits an interval, not a p-value. Explicitly `confirmatory: false` in the schema. |
| A3 | No | S is descriptive, has no pre-registered threshold, and cannot alter a rival's signature match (§2.6). |
| A4 | No | The two secondary tasks were already outside the family in revision 1 (§3.1, §5.1 row A12) and remain outside it. **Promoting them would take the family from 16 to 24 and force an alpha change — this was checked and explicitly not done.** |
| A5 | No | Contains no analysis. |
...
## 2.3 Generations per cell, batching, seeding — **AMENDED (A1)**

- **20 generations per batch, 10 batches per cell, 200 generations per cell.**
  *(Revision 1: 5 batches, 100 per cell.)*
- **30 cells × 200 = 6,000 generations.** No architecture is trained.
  *(Revision 1: 3,000.)*
...
- Diversity is computed **within batch**; a cell reports **mean ± std across its
  10 batches**. This is the same estimand discipline as E2 (§3.4): repeat the
  procedure, report mean and standard deviation across repetitions.

### Why 10 and not 5 — the discriminating load sits on the change columns

The five rivals are separated primarily by the two **change** columns (`bf16`,
`high temp`), not by the three level columns. Three of the five signatures agree
on all three level columns being collapsed or diverse together; what splits
`quantisation` from `genuine prior`, and `decoding` from `repair artifact`, is a
ΔD of at least 0.25·D_rand against a "no change" band of 0.10·D_rand. **ΔD is a
difference of two cell means, so it carries roughly √2 times the noise of a
single cell mean**, and estimating it across 5 batch means is not adequate to
resolve a 0.10·D_rand boundary.

There is a second, harder reason, found while implementing this amendment and
recorded here because it is a defect in revision 1 rather than a refinement:

> **At 5 batches the paired permutation test in revision 1 could never have
> reached the declared alpha.** A paired sign-flip permutation test over *B*
> paired batch differences has exactly 2^*B* equally likely sign assignments. At
> *B* = 5 that is 32, so the smallest attainable two-sided p-value is
> 2/32 = **0.0625** — twenty times larger than ALPHA = 0.003125. Contrasts X1–X4
> were structurally incapable of rejecting, whatever the data showed.
>
> At *B* = 10 there are 2^10 = 1024 assignments and the smallest attainable
> two-sided p-value is 2/1024 = **0.001953**, which clears ALPHA. **Ten is the
> minimum batch count at which E1's confirmatory tests are decidable at all**,
> and even at ten the test is close to saturated: significance requires all ten
> paired differences to share a sign *and* the observed mean difference to exceed
> that of all 1,022 other sign patterns. This is a real limitation of the paired
> form and is why A2's bootstrap exists as a resolution check (§2.4.5), and why
> §5.2 requires the attainable minimum p-value to be reported next to every
> paired result.

The unpaired form used for a frontier cell whose seeds are not honoured does not
have this problem: a two-sample permutation over 20 values split 10/10 has
C(20,10) = 184,756 assignments and a minimum two-sided p of about 1.1 × 10⁻⁵.

### Seeding scheme — **AMENDED (A1)**

A single frozen seed vector

```
S = [7001, 7002, 7003, 7004, 7005, 7006, 7007, 7008, 7009, 7010]
```
...
the results file. The first five entries are unchanged from revision 1, so any
work already planned against them is unaffected.

For the frontier API, if the provider does not honour a seed parameter, the cell
records `seed_honoured: false` and its ten batches are treated as unpaired
...
### 2.4.5 Secondary outcome — within-cell bootstrap interval on D — **NEW (A2)**
...
side of a threshold?" — a question the discrete paired test (§2.3) cannot answer.

**Pooled cell statistic.** `D_cell_pooled` = mean of *d* over all C(N, 2)
unordered pairs of the cell's *N* parseable generations, pooling across all ten
batches. This is a **different estimand** from `D_mean` (the mean of the ten
within-batch D values): it additionally includes cross-batch pairs, and because
batches differ only by sampler seed, those pairs are legitimate. Both are
...
on ΔD. This interval is what determines whether ΔD can be resolved against the
0.10·D_rand and 0.25·D_rand boundaries at all.
...
and the signature match for that cell is scored `indeterminate` under the same
rule as §2.6's parse-rate provision — this is the one place the bootstrap alters
an outcome, and it can only ever move a cell **toward** `indeterminate`, never
toward a match.

### 2.4.6 Secondary outcome — parse-free surface diversity **S** — **NEW (A3)**

**Purpose, and its only purpose.** Revision 1 §2.6 records that free-prose
pre-repair diversity is conditioned on the generation having parsed at all, a
non-random subset, and that free-prose is the condition most likely to fail
parsing. That makes the cell most load-bearing for separating `format tax` from
`genuine prior` also the least reliable. **S bounds that selection effect by
measuring diversity over the full generation set, including the failures.** It
...
6. **Cell statistic** — `S_mean ± S_std` across the ten batches, plus
...
**Free-prose selection effect — pre-registered handling. AMENDED (A3).**
Free-prose pre-repair diversity is conditioned on the generation having parsed at
all, which is a non-random subset. There is no clean fix; a permissive
prose-to-fields extractor was considered and rejected, because it is a second
instrument with its own artifact and would defeat the purpose of the experiment.
The handling is therefore two-part:

1. **The rule, unchanged from revision 1.** The free-prose parse rate is reported
   adjacent to every free-prose diversity number, and **if the free-prose parse
   rate is below 50%, that cell's diversity estimate is declared unreliable and
   the free-prose column is scored `indeterminate` for every rival** — neither
   matched nor unmatched — with the maximum achievable signature score dropping
   to 4 and the ≥4-of-5 rule reading ≥3-of-4 accordingly.
2. **The bound, new in revision 2.** `S` (§2.4.6) is computed on the full
   generation set including parse failures, and is reported alongside. It
   *characterises* how much of the observed concentration could be a selection
   artifact. **It does not change rule 1.** A column ruled `indeterminate` by the
   parse-rate test stays `indeterminate` whatever S shows; a column not so ruled
   is not re-opened by S either.

**Bootstrap straddling a boundary — new in revision 2.** Where the §2.4.5 BCa
interval on a cell's D, or on a contrast's ΔD, straddles the classification
boundary that decides its label, that cell is scored `indeterminate` for
signature-matching purposes and the interval is reported. This can only reduce
the number of matched cells; it can never create a match.
...
- A batch with fewer than 2 parseable generations yields `D = null`. It is
  ...
  batches and the null count is reported beside it. A cell with **≥6 null batches
  out of 10** is reported as `insufficient` and contributes no signature match.
  *(Revision 1 set this at ≥3 of 5; the proportion is unchanged.)*
...
- **S degenerate cases — new in revision 2.** A generation of fewer than 3 tokens
  after normalisation has an empty 3-gram set. A pair of two empty sets is defined
  as `d_S = 0` (both identical and empty); a pair of one empty and one non-empty
  set is `d_S = 1`. The count of empty-3-gram generations is recorded per batch,
  and a batch in which more than half the generations have empty 3-gram sets
  yields `S = null`, recorded and never imputed.
...
**Why all three are retained, recorded so the question is settled and does not
reopen.** The substrate is a table lookup: adding a task adds **no training and
no evaluation cost whatsoever**, only generation on a locally-served model. Under
that cost structure, multi-task coverage is **the cheapest external validity
available anywhere in this design** — it is the difference between a claim about
CIFAR-100 and a claim about a search space's three standard tasks, bought with
local GPU hours and nothing else. A single-task result invites precisely the
generalisation objection that Yang et al. and Li & Talwalkar press on the NAS
literature, and answering it later would cost far more than running it now.
Dropping the secondary tasks is therefore **not** a scope reduction this plan
recognises; see §8.
...
replicate is the sole test of **C5** and its result is confirmatory in the
argumentative sense but is evaluated descriptively (§5.3), so it does not enter
the 16-test family.
...
## 3.5 Power / MDE and stopping rule — **budget AMENDED (A1)**
...
for |δ| ≥ 0.62, equivalently a location shift of ≈1.05 pooled SDs. E2's
permutation test is the **unpaired** two-sample form over C(40,20) ≈ 1.4 × 10¹¹
assignments, so it carries none of the discreteness problem that constrains E1's
paired form (§2.3).
...
**Generation budget — revision 2.**

| Component | Generations |
|---|---|
| E1 — 30 cells × 10 batches × 20 | **6,000** |
| E2 primary — 4 arms × 20 runs × 20 proposals × 3 tasks | 4,800 |
| E2 conditional replicate — 4 arms × 20 runs × 20 proposals × 1 task | 1,600 |
| E3 | 0 |
| **Total** | **12,400 generations, zero training runs** |

*(Revision 1: 3,000 + 4,800 + 1,600 = 9,400.)* The increase is entirely A1, and
is the cost of making E1's confirmatory tests decidable at all (§2.3).

---

# 4. Experiment E3 — the proxy size-confound replication (Case B)

*(Unchanged from revision 1 in its entirety.)*
...
| A2 | E1 | D per cell: mean ± std across **10** batches | descriptive |
...
| **A16** | E1 | `D_cell_pooled` + BCa 95% bootstrap interval, per cell × stage *(new, A2)* | descriptive |
| **A17** | E1 | BCa 95% bootstrap interval on ΔD for the `bf16` and `high temp` contrasts *(new, A2)* | descriptive |
| **A18** | E1 | `S` per batch and `S_mean ± S_std` per cell, plus `S_cell_pooled` with BCa interval *(new, A3)* | descriptive |
| **A19** | E1 | boundary-straddle flags: which cells were scored `indeterminate` by §2.6's bootstrap rule *(new, A2)* | classification |

**Confirmatory tests: X1–X4 across 3 models = 12, plus Y1–Y4 = 4. Total 16.**
A16–A19 are descriptive and carry no p-value. **The count is unchanged from
revision 1.**

## 5.2 Multiplicity correction — **UNCHANGED, reaffirmed (A6)**
...
No amendment in revision 2 adds, removes, or reclassifies a confirmatory test.
...
A3–A19 are descriptive, classification, or exploratory and carry no p-value.
...
**Discreteness reporting — new in revision 2.** Every paired permutation result
(X1–X4) is emitted with `min_attainable_p`, the smallest two-sided p-value its
permutation distribution admits given the number of usable paired batches:
2/2^B for B paired differences. At the planned B = 10 this is 0.001953. **A
non-significant paired result must be read against this number**: where
`min_attainable_p ≥ alpha`, the test could not have rejected and the result is
reported as *undecidable by this test*, not as evidence of no effect. The emitter
writes the value; the manuscript reports it wherever a paired p-value appears.
...
The same applies to every statistic added in revision 2: A16–A19 emit intervals
and flags, never p-values, and adding a p-value to any of them would be a
deviation requiring a `DEVIATIONS.md` entry **and** — because it would change
FAMILY_SIZE — a revision 3, not an alpha adjustment.
...
**`schema_version` is `1.1.0` in revision 2** (revision 1 specified `1.0.0`); the
change is additive only — every revision-1 key is retained with the same meaning,
and the new keys are marked below.
...
```json
{
  ...
  "experiment": "E1",
  "plan_revision": 2,
  "plan_sha256": "<SHA-256 of EXPERIMENT_PLAN_R2.md at freeze>",
  "plan_supersedes_sha256": "aeb174ffb008252368cc7fbcb121bd0fa0642f2fa4f3ec70228256920bfbad3d",
  ...
  "statistics": [
    {
      ...
      "paired": true, "n_pairs": 10, "n_permutations": 1024,
      "min_attainable_p": 0.001953,
...
New in `1.1.0`: `plan_revision`, `plan_supersedes_sha256`, `n_batches_per_cell`,
`bootstrap`, `seed_honoured`, `S`, `empty_trigram_count`, `D_*_pooled`,
`D_*_pooled_ci95`, `S_mean`, `S_std`, `S_pooled`, `S_pooled_ci95`,
`null_S_batches`, `boundary_straddle`, `min_attainable_p`, `delta_ci95_boot`,
`indeterminate_columns`.
...
*(Unchanged from revision 1. Rule 4 is the authority under which this revision
was written.)*

`DEVIATIONS.md` carries no entries as of revision 2's hash.
...
away, and the runway was flagged as a risk against a plan then specifying ~9,400
generations. **That consideration has been examined and rejected as an input to
scope.**
...
1. **The chosen venue is non-archival** (`VENUE.md`; the fallback is explicitly
   non-archival, and the primary's status is being verified at S6). Missing a
   cycle therefore costs **nothing** — no priority, no publication window, no
   claim to the result. The work is not consumed by not submitting it.
...
3. **The failure this whole project exists to correct was produced by exactly
   ...
   by a design. Repeating that trade would forfeit the only thing this rebuild
   has to offer.
4. **The amendments in revision 2 raise the generation budget by 3,000 and were
   adopted anyway**, because A1 is what makes E1's confirmatory tests decidable
   at all (§2.3). That is the precedent this section fixes: **when rigour and the
...
at S4, or at any later stage, on schedule grounds:

- dropping E1 cells, batches, or generations per batch;
- dropping E2's secondary tasks (§3.1) — retained on the reasoning recorded
  there, and their marginal cost is generation only;
- dropping E2's conditional replicate (§3.3), which is the sole test of C5;
- reducing R below the value S3's power simulation confirms;
- narrowing the confirmatory family, which would change ALPHA (§5.2).

A genuine scope change on **methodological** grounds remains available at any
time — this section constrains the *reason*, not the possibility. Such a change
is a `DEVIATIONS.md` entry logged before the affected work, or a revision 3 where
it alters FAMILY_SIZE.
...
their own stages rather than here: fetching and byte-verifying `neurips_2026.sty`
from two mirrors (S6), and closing OA-37 and OA-38 before S6 writing. Neither
touches E1, E2 or E3.
